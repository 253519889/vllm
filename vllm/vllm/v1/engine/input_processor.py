...
def _get_prompt_token_length_breakdown(
    ...
    """Compute text token count and visual token count from total prompt length.

    Visual tokens are those in image/video placeholder ranges. The rest are
    text tokens. Useful for debugging VL models (e.g. Qwen3-VL-4B-Instruct).
    """
    num_visual = sum(
        r.length
        for mod in _VISUAL_MODALITIES
        for r in mm_placeholders.get(mod, [])
    )
    num_text = prompt_len - num_visual
    return num_text, num_visual
...
class InputProcessor:
    ...
    def process_inputs(
        ...
        if decoder_inputs["type"] == "multimodal":
            ...
            # Log text vs visual token lengths for VL models (e.g. Qwen3-VL-4B).
            if prompt_token_ids is not None:
                total_len = len(prompt_token_ids)
                num_text, num_visual = _get_prompt_token_length_breakdown(
                    total_len, decoder_mm_positions
                )
                logger.info(
                    "Prompt token lengths: total=%d, text=%d, visual=%d "
                    "(request_id=%s)",
                    total_len,
                    num_text,
                    num_visual,
